...
class OutboxWorker:
    ...
    async def _send_notification(self, event) -> bool:
        ...
        async with CapashinoClient(
            ...
            api_key=settings.CAPASHINO_API_TOKEN
        ) as client:
                        ...
                        for event in pending:
                            try:
                                ...
                                if success:
                                    ...
                                    await repo.mark_failed(event.id, "Capashino API returned error")
                                    logger.warning(f"Outbox event {event.id} failed, will retry later")
                                    
                            except Exception as e:
                                error_msg = str(e)
                                logger.error(f"Error processing outbox event {event.id}: {error_msg}")
