...
class OutboxRepository:
    ...
    async def create(self, event_type: str, payload: dict) -> str:
        ...
        logger.debug(f"Outbox event created: {event_id}, type: {event_type}")
        return event_id

    async def get_pending(self, limit: int = 10) -> list[OutboxEvent]:
        """Получить неотправленные события (не более limit)"""
        query = (
            select(outbox_tbl)
    ...
    async def mark_failed(self, event_id: str, error: str):
        """Пометить событие как неудачное (увеличить attempts)"""
        stmt = (
            update(outbox_tbl)
            .where(outbox_tbl.c.id == event_id)
            .values(
                status="pending",
                attempts=outbox_tbl.c.attempts + 1,
                last_error=error,
    ...
    async def mark_final_failed(self, event_id: str, error: str):
        """Пометить событие как окончательно неудачное (после всех попыток)"""
        stmt = (
            update(outbox_tbl)
            .where(outbox_tbl.c.id == event_id)
            .values(status="failed", last_error=error, updated_at=datetime.now())
        )
        await self.session.execute(stmt)
        await self.session.commit()
        logger.error(f"Outbox event final failed: {event_id}, error: {error}")
